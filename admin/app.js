...
function renderMetrics() {
  ...
  // Calculate today's revenue from transaction log
  const todayRevenue = state.transactions.reduce((acc, trans) => acc + trans.amountPaid, 0);
  
  // Set UI elements
  document.getElementById('metric-available').textContent = totalAvailable;
  document.getElementById('metric-reserved').textContent = totalReserved;
  document.getElementById('metric-revenue').textContent = todayRevenue;
  
  // Update Master SVG Circular Gauge
  document.getElementById('occupancy-pct').textContent = `${occupancyPct}%`;
...
function updateCheckoutDetails() {
  ...
  document.getElementById('cash-amt-invoice').textContent = bill.fee;
  
  // Cash Change Refund calculation update
  updateCashRefund();
}
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // 5. Sidebar Select dynamic lists trigger
  document.getElementById('vehicle-type').addEventListener('change', populateFormSlotDropdown);
  document.getElementById('select-floor').addEventListener('change', populateFormSlotDropdown);

  // 6. Action selector update in form (Show Name field for reservation)
  document.getElementById('action-type').addEventListener('change', function() {
    const isReserve = this.value === 'reserve';
    document.getElementById('reservation-name-group').style.display = isReserve ? 'flex' : 'none';
    document.getElementById('btn-submit-action').textContent = isReserve ? 'Confirm Reservation' : 'Execute Check-In';
  });
  ...
  // 8. Sidebar Form Submit Handler
  document.getElementById('quick-action-form').addEventListener('submit', function(e) {
    e.preventDefault();
    const plate = document.getElementById('vehicle-plate').value;
    const type = document.getElementById('vehicle-type').value;
    const action = document.getElementById('action-type').value;
    const floor = document.getElementById('select-floor').value;
    let slotId = document.getElementById('select-slot').value;

    // Auto assigned slot selector logic
    if (slotId === 'auto') {
      const avail = state.slots[floor].find(s => s.type === type && s.status === 'Available');
      if (!avail) {
        alert(`Sorry, no available slots for ${type}s on Floor ${floor}!`);
        return;
      }
      slotId = avail.id;
    }
    ...
    if (action === 'checkin') {
      checkInVehicle(plate, type, slotId);
      showNotification(`Vehicle ${plate.toUpperCase()} checked in to slot ${slotId.split('-')[1]}.`);
    } else {
      const name = document.getElementById('customer-name').value || 'Reserved Guest';
      reserveSlot(slotId, plate, name);
      showNotification(`Slot ${slotId.split('-')[1]} reserved for ${name}.`);
    }

    this.reset();
    document.getElementById('reservation-name-group').style.display = 'none';
    document.getElementById('btn-submit-action').textContent = 'Execute Check-In';
    renderAll();
  });
  ...
  // 10. Global search dynamic listener
  document.getElementById('global-search').addEventListener('input', function() {
    handleGlobalSearch(this.value);
  });

  // 11. Custom Settings Form Submit
  document.getElementById('settings-form').addEventListener('submit', function(e) {
    e.preventDefault();
    
    const rateCar = parseInt(document.getElementById('rate-car').value) || 0;
    const rateBike = parseInt(document.getElementById('rate-bike').value) || 0;
    const capCar = parseInt(document.getElementById('capacity-car').value) || 0;
    const capBike = parseInt(document.getElementById('capacity-bike').value) || 0;
    
    if (capCar + capBike > 80) {
      alert("Error: Total floor slots capacity (Cars + Bikes) cannot exceed 80 to prevent display issues!");
      return;
    }

    state.rates.Car = rateCar;
    state.rates.Bike = rateBike;
    
    // Check if slots capacities changed. If so, rebuild database layout
    if (state.capacities.Car !== capCar || state.capacities.Bike !== capBike) {
      if (confirm("Changing capacity resizes your garage slots grid. Existing parking slot states will be rebuilt. Do you wish to proceed?")) {
        state.capacities.Car = capCar;
        state.capacities.Bike = capBike;
        createBlankDatabase();
      }
    }
  ...
  // Dynamic slot calculations update
  const calcCapTotals = () => {
    const cc = parseInt(document.getElementById('capacity-car').value) || 0;
    const cb = parseInt(document.getElementById('capacity-bike').value) || 0;
    document.getElementById('calculated-total-slots').textContent = `${cc + cb} slots`;
  };
  document.getElementById('capacity-car').addEventListener('input', calcCapTotals);
  document.getElementById('capacity-bike').addEventListener('input', calcCapTotals);

  // Reset database button trigger
  document.getElementById('btn-reset-db').addEventListener('click', () => {
    if (confirm("Are you sure you want to WIPE out all active parking slots and transactions log? This cannot be undone!")) {
      resetDatabase();
    }
  });

  // Mock demo state trigger
  ...
  document.getElementById('btn-export-csv').addEventListener('click', () => {
    ...
    let csv = 'Receipt ID,Plate Number,Type,Floor,Slot,Duration,Amount Paid,Payment Method,Check-Out Date\n';
    state.transactions.forEach(t => {
      csv += `"${t.receiptId}","${t.plate}","${t.type}","Floor ${t.floor}","${t.slotNumber}","${t.duration}","₹${t.amountPaid}","${t.paymentMethod}","${t.checkOutTime}"\n`;
    });
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `parkpro-transactions-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    showNotification("CSV export successfully generated.");
  });
  ...
  // Dynamic cash input change calculation trigger
  document.getElementById('cash-given').addEventListener('input', updateCashRefund);

  // 13. CONSOLE ROLE TOGGLES AND ACTIONS
